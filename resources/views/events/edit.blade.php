...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6 flex justify-between items-center">
        <h1 class="text-3xl font-bold text-gray-900">Edit Event: {{ $event->title }}</h1>
        <a href="{{ route('events.show', $event) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" 
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">{{ old('description', $event->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="start_date" id="start_date" 
                        value="{{ old('start_date', $event->start_date->format('Y-m-d\TH:i:s')) }}" 
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                        required>
                    @error('start_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="end_date" id="end_date" 
                        value="{{ old('end_date', $event->end_date->format('Y-m-d\TH:i:s')) }}" 
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                        required>
                    @error('end_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="col-span-2">
                    ...
                    @if($event->image)
                        <div class="mt-2 mb-3">
                            <div class="flex items-center space-x-4">
                                <img src="{{ Storage::url($event->image) }}" alt="{{ $event->title }}" class="h-20 w-32 object-cover rounded">
                    ...
                    <p class="mt-1 text-sm text-gray-500">PNG, JPG or JPEG up to 2MB. Leave empty to keep the current image.</p>
                    @error('image')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
